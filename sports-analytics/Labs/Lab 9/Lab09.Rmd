...
*0 results in a lose, 1 results in a win, and 0.5 results in a game ending in a tie.*
...
```{r}
icehockey %>% count(visitor)
```

*Alaska Anchorage is set as the reference team. Looking at the model, they appear to be an average team.*
...
```{r}
strength <- tidy(standardBT)

strength %>%
  arrange(-estimate)
```

*BTM identifies Denver, Miami, and Wisconsin as the best teams. Looking at the internet, it seems that Boston College won the league, with Wisconsin getting second, and RIT and Miami tying for 3rd.* 

5. Identify the biggest mismatch in teams, and what the probability is of the worse team winning that game.

*The largest mismatch would be Denver versus American International, and the probability that the team would be 0.01.*
